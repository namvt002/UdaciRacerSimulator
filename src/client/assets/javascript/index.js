...
async function handleCreateRace() {
	...
	if(!track_id || !player_id) {
		alert("Please choose a track and racer")
		return
	}

	const response_data = await createRace(player_id, track_id)
	store.race_id = parseInt(response_data.ID)

	renderAt('#race', renderRaceStartView(response_data.Track, response_data.Race))
	await runCountdown();
...
async function getTracks() {
	...
	try{
		const response_data = await fetch(`${SERVER}/api/tracks`,{
			method: 'GET',
			dataType: 'jsonp',
			...defaultFetchOpts(),
		})
		const data_track = await response_data.json();
		return data_track
	}catch(err){
...
async function getRacers() {
	...
	try{
		const res_data = await fetch(`${SERVER}/api/cars`,{
			method: 'GET',
			dataType: 'jsonp',
			...defaultFetchOpts(),
		})
